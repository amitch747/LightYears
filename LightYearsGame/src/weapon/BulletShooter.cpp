...
namespace ly
{
	...
	{
		sf::Vector2f ownderForwardDir = GetOwner()->GetActorForwardDirection();
		...
		newBullet.lock()->SetActorLocation(GetOwner()->GetActorLocation() + ownderForwardDir * mLocalPositionOffset.x + ownerRightDir * mLocalPositionOffset.y);
